...
@section('title', 'Редагувати товар у замовленні')

@section('content_header')
    <h1 class="m-0 text-dark">Редагувати товар у замовленні</h1>
@stop

@section('content')
<div class="row justify-content-center">
    <div class="col-12">
        <form action="{{ route('order-products.update', [$order->id, $order_product->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-12">
                        ...
                        <input type="hidden" name="product_id" id="product_id" value="{{ $order_product->id }}">
                        {{-- Количество товара --}}
                        <div class="form-group">
                            <label for="count" class="red-star">Кількість</label>
                            <input type="number" min="1" step="1"
                                class="form-control @error('count') is-invalid @enderror" id="count"
                                name="count" value="{{ old('count', $order_product->count) }}"
...
<script>
    $(document).ready(function() {
        ...
        //Редактируемый продукт в заказе
        var order_product = @json($order_product);
        //Old values
        ...
        table.one('draw', function () {
            if (old.product_id) {
                ...
                table.row('#id'+ order_product.product_id).show().draw(false);
                table.row('#id'+ order_product.product_id).select();
            };
        });
                    ...
                    option => {
                        ...
                        if (selected[option]) {
                            ...
                            selected[option].forEach(opt => {
                                if (old[option]) {
                                    var current_value = old[option];
                                } else {
                                    var current_value = order_product[option];
                                };
