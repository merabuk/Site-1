<header>
    <div class="menu-phone-block d-none d-md-block">
        <div class="container">
            <ul>
                <li>
                    <i class="icon icon-phone-white"></i>
                    <a href="tel:+380504955230">(050) 495-52-30</a>
                </li>
                {{-- <li>
                    <i class="icon icon-phone-white"></i>
                    <a href="tel:+380487770700">(048) 777-07-00</a>
                </li> --}}
            </ul>
        </div>
    </div>
        ...
        <div class="container">
            <ul class="main-menu-top">
                ...
                <li class="menu-user-list">
                    <ul>
                        <sign-in-sign-up-li :switch-disable="switchDisable"
                            :route-user="url"
                            :route-user-name="textForUrl"
                            :get-user-name="userName"
                            old-name="{{ session()->get('username') }}"
                            v-on:logout-success="logout"></sign-in-sign-up-li>
                        <favorite-li :switch-disable="switchDisable"
                            path="{{ route('product-favorit') }}"></favorite-li>
                        <cart-module-home :on-cart-page="{{ (Request::is('cart')) ? 'true' : 'false' }}"
                            cart-route="{{ route('cart') }}"></cart-module-home>
                        @if (Request::is('cart'))
                            <li></li>
                        @endif
                    </ul>
    ...
    <div class="menu-bottom-block">
        <div class="container">
            <input id="sandwich-toggle" type="checkbox" class="d-none"/>
            <nav class="sidenav bg-dark">
                <ul class="sidenav-header d-lg-none">
                    <li class="mobile-logo">
                        <a href="/">
                            <i class="icon icon-sidenav-logo"></i>
                        </a>
                    </li>
                    <li>
                        <label for="sandwich-toggle">
                            <i class="icon icon-quit"></i>
                        </label>
                    </li>
                </ul>
                <ul class="sidenav-main d-lg-none">
                    <sign-in-sign-up-li-mobile :switch-disable="switchDisable"
                        :route-user="url"
                        :route-user-name="textForUrl"
                        :get-user-name="userName"
                        old-name="{{ session()->get('username') }}"></sign-in-sign-up-li-mobile>
                    <favorite-li-mobile :switch-disable="switchDisable"
                        path="{{ route('product-favorit') }}"></favorite-li-mobile>
                    <li>
                        <a href="{{ route('cart') }}" class="nav-link">
                            <i class="icon icon-basket mr-3"></i>
                            <span>Кошик</span>
                        </a>
                    </li>
                    <logout-li-mobile :switch-disable="switchDisable"
                        v-on:logout-success="logout"></logout-li-mobile>
                </ul>
                <ul class="sidenav-main main-catalog">
                    <li>
                        <div class="catalog-header">
                            <a href="" class="nav-link" data-toggle="collapse" data-target="#catalog-wrapper"
                                aria-expanded="false" aria-controls="catalog-wrapper" onclick="event.preventDefault()">
                                <i class="icon icon-menu-button mr-3"></i>
                            </a>
                            <a class="nav-link nav-link-uppercase catalog-pl @if(url()->current() == route('catalog-product')) bg-lg-orange @endif"
                                href="{{ route('catalog-product') }}">
                                <span>каталог</span>
                            </a>
                        </div>
                        <div class="collapse sidebar-collapse" id="catalog-wrapper">
                            <ul class="sidenav-header d-lg-none">
                                <li class="prev-action" id="history" data-default="#catalog-wrapper" data-current>
                            ...
                            <ul class="accordion bg-light" id="catalogAccordion">
                                @include('frontend.includes.menu')
                            </ul>
                        </div>
                    </li>
                </ul>
                <ul class="sidenav-main main-list">
                    <li>
                        <a href="{{ route('product-new') }}" class="nav-link @if(url()->current() == route('product-new')) bg-lg-orange @endif">Новинки</a>
                    </li>
                    <li>
                        <a href="/page-pay-shipping" class="nav-link @if(url()->current() == route('main').'/page-pay-shipping') bg-lg-orange @endif">Оплата і доставка</a>
                    </li>
                    <li>
                        <a href="/info-company" class="nav-link @if(url()->current() == route('main').'/info-company') bg-lg-orange @endif">Про компанію</a>
                    </li>
                    <li>
                        <a href="/contact" class="nav-link @if(url()->current() == route('main').'/contact') bg-lg-orange @endif">Контакти</a>
                    </li>
                    <li>
                        <a href="/info-question" class="nav-link @if(url()->current() == route('main').'/info-question') bg-lg-orange @endif">Питання та відповіді</a>
                    </li>
                    <li>
                        <a href="{{ route('product-sale') }}" class="nav-link @if(url()->current() == route('product-sale')) bg-lg-orange @endif">Розпродаж</a>
                    </li>
                </ul>
            </nav>
        </div>
