<template>
    <div>
        <a href="" class="one-click-buy" v-on:click.prevent="checkOneAttribute">
            <span class="icon icon-bag"></span>
            <span>Купити в 1 клік</span>
        </a>
        <div class="modal fade" :id="modalId" tabindex="-1" role="dialog" aria-labelledby="buyOneClickPreviewModalLabel" aria-hidden="true">
                        ...
                        <div class="row">
                            <div class="col-12 col-md">
                                <div class="form-check terms">
                                    <input type="checkbox" :id="'terms-check2'+product.id" required v-model="termsCheck"
                                        :class="{ 'is-invalid' : termsValid }">
                                    <label :for="'terms-check2'+product.id" class="terms-check2-sapn">
                                        <span class="d-inline-block">Я згоден на <a href="/terms-check" target="_blank">обробку персональних даних</a></span>
                                    </label>
    ...
    export default {
        ...
        data() {
            return {
                ...
                termsCheck: false,
                termsValid: false,
                modalId: 'buyOneClickPreviewModal-'+this.product.id,
                modalTarget: '#buyOneClickPreviewModal-'+this.product.id,
                captchaSrc: '',
                captchaId: 'captcha-'+this.product.id,
            }
        },
        watch: {
        ...
        methods: {
            //проверка на наличие у товара всех атрибутов по 1
            checkOneAttribute() {
                this.reloadCaptcha();
                this.form.formData = [];
                ...
                $.each(attr, (index, value) => {
                    if (this.product[value].length != 0  && this.product[value].length == 1) {
                        ...
                        this.form.formData.push({name: value, value: this.product[value][0]});
                ...
                if (this.form.formData.length == 0 || sizeMultiple) {
                    $('#Modal-warning-'+this.product.id).modal('show');
                } else {
                    this.form.formData.push({name: 'product_id', value: this.product.id});
                    this.form.formData.push({name: 'count', value: 1});
