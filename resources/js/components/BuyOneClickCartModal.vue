<template>
    <div class="modal fade" id="buyOneClickCartModal" tabindex="-1" role="dialog" aria-labelledby="buyOneClickCartModalLabel" aria-hidden="true">
                    ...
                    <!--<form>-->
                        ...
                        <div class="form-group row">
                            <div class="col-12 col-md">
                                <span><img id="captcha-cart-img" :src="clickCaptchaSrc"></span>
                        ...
                        <div class="row">
                            <div class="col-12 col-md">
                                <div class="form-check terms">
                                    <input type="checkbox" id="terms-check3" required v-model="termsCheck"
                                        :class="{ 'is-invalid' : termsValid }">
                                    <label for="terms-check3" class="terms-check2-sapn">
                                    ...
                                    <div v-if="errors.load" class="d-block invalid-feedback">{{ errors.load }}</div>
                                </div>
                            </div>
                        </div>
        ...
        methods: {
            ...
            sendToOrder() {
                if (this.termsCheck) {
                    ...
                    this.errors.load = '';
                    axios.post(window.origin+'/orders/oneclick/cart', this.form)
                    //если удачно
                    .then((response) => {
                        ...
                        } else if(response.data.result && response.data.result == 'empty') {
                            this.errors.load = 'Будь ласка додайте товари в кошик!';
                        } else {
                            this.errors.load = 'Вибачте заказ не створено. Спробуйте ще раз.';
                        };
                    })
                    //если неудачно
                    .catch(error => {
